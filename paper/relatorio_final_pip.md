...
**Distribuição e outliers:** os valores acima são médias; a Figura abaixo mostra a distribuição completa (150 trials de custo A*, 300 de custo BC por mundo, e os 1.500 trials de ρ_local). O custo do A* real não é constante entre trials — em `very_dense`, 14 dos 150 trials (9,3%) são outliers estatísticos (acima de Q3 + 1,5×IQR = 33,3 ms), chegando a 51,9 ms no pior caso, quase o dobro da mediana (28,1 ms). Esses outliers correspondem a pares start-goal que exigem busca mais extensa na grade (mais nós expandidos antes de alcançar o objetivo), um comportamento esperado de busca em grafo e não um artefato de medição. O custo do BC, por contraste, tem variância desprezível em todos os mundos (desvio padrão < 0,05 ms), consistente com ser um único *forward pass* de custo fixo independente da geometria do trial. A distribuição de ρ_local (painel c) mostra que a maioria dos trials em `dense`/`very_dense` já nasce acima do limiar ρ*=0,30 original, explicando por que o ρ-criterion raramente aciona o A* nesses mundos (Seção 4).

**Parâmetros da revalidação e por que foram escolhidos:** a grade de rasterização do A* usa resolução de 0,08 m (compromisso entre fidelidade geométrica e tempo de busca — resoluções mais finas aumentam o número de nós sem mudar qualitativamente os resultados, testado informalmente) e margem de segurança extra de 0,08 m além do raio do robô (necessária porque o controlador de perseguição de caminho corta cantos entre waypoints discretos; sem essa margem, a taxa de colisão do A* real chegava a 40% em `very_dense`, um artefato do controlador, não do planejador — com a margem, caiu para a faixa 0-9% reportada acima). O número de trials (1.500) iguala a escala do Monte Carlo original para permitir comparação direta.

**Achado complementar — o mesmo tratamento aplicado ao multiagente:** o achado "A* independente supera SAC independente" de uma sessão anterior (100%/0% de sucesso/colisão vs. ~38%/100%) também usava a política de linha reta em vez de A* real. Regerado com A* real (busca em grade por agente, sem coordenação, mesmo desenho experimental original, N=4, 20 trials/mundo): o A* real continua vencendo em taxa de chegada ao objetivo (71%/55%/35% vs. 57%/36%/25% do SAC, em sparse/dense/very_dense), mas **a taxa de colisão entre robôs também é alta para o A* real** (75%/80%/65%, contra 60%/50%/60% do SAC) — bem diferente da alegação original de 0% de colisão. A explicação: A* real segue o caminho mais curto de forma rígida, sem qualquer desvio dinâmico de outros robôs (o planejamento ignora os vizinhos, por desenho); a política de linha reta original, por parar quando desalinhada (`v_norm=0` se `|Δθ|≥0,6` rad), acabava reduzindo colisões por um efeito colateral do controlador, não por qualidade de planejamento. Dado real: nenhum planejador independente sem coordenação evita colisão entre robôs de forma confiável — reforçando, com dados corretos, a mesma conclusão da Seção 3.4 sobre a necessidade de MARL para coordenação (a alegação original de "0% de colisão" estava incorreta e nunca foi citada nos documentos formais desta IC).

**Evidência empírica preliminar de MARL (reward compartilhada):** para testar diretamente se treino conjunto resolve o problema de coordenação identificado acima, implementamos uma versão simplificada de MARL no gêmeo 2D — uma política única, centralizada (não descentralizada tipo CTDE/QMIX/MAPPO), que recebe a observação concatenada dos N agentes e produz as N ações conjuntamente, treinada com PPO e reward igual à média das rewards individuais, incluindo uma penalidade de colisão inter-robô **compartilhada** (aplicada aos dois agentes envolvidos, não a um "culpado" arbitrário) — a peça estrutural que falta no RL independente. Com N=4 no mundo `sparse`: em 150 mil passos, a taxa de chegada ao goal era 25% mas a colisão inter-robô já era 0%; em 600 mil passos, a taxa de goal subiu para 50% (aproximando-se dos 57% do RL independente) **mantendo colisão inter-robô em 0%** (contra 60% do RL independente) em ambos os pontos de treino. A tendência — goal rate subindo com mais treino, colisão permanecendo nula desde cedo — é evidência real de que a reward compartilhada resolve estruturalmente o problema de coordenação, ao custo de um treino mais lento (o espaço de ação conjunto de 8 dimensões, com reward diluída por 4 agentes, é mais difícil de explorar que o problema single-agent de 2 dimensões). Esta é uma simplificação de MARL — treino e execução centralizados, não a arquitetura descentralizada completa (QMIX, MADDPG, MAPPO) declarada como trabalho futuro — mas é a primeira evidência empírica desta IC de que treino conjunto com recompensa compartilhada, e não apenas a formalização teórica, resolve o problema identificado.
...
**H1 — reformulada com dados reais: confirmada como argumento de custo computacional, não de taxa de sucesso.** Na validação Monte Carlo com modelos calibrados (Seção 3.2), o framework obteve 85,3% de taxa de sucesso contra 76% do melhor método fixo — diferença de 9,3 pontos percentuais estatisticamente significativa (p=0,0203, 150 trials), mas sobre planejadores mock. A validação com planejadores reais em Gazebo, planejada como o resultado principal da Fase 2, não foi concluída nesta IC (Seção 3.3 — ver Limitações). Para reduzir essa dependência de mock sem precisar do Gazebo, implementamos um A* real (busca em grade, não a linha reta usada nas comparações multiagente anteriores) no gêmeo 2D e revalidamos H1 com 1.500 trials reais, pareados, de pool misto de densidade (Seção 3.3 — mesma escala do Monte Carlo original, permitindo comparação direta): **a motivação original de H1 — que a política aprendida supera o A* em taxa de sucesso sob alta densidade — não se sustentou** (A* real: 88,7% de sucesso pareado; ρ-criterion real: 84,1%; regret real de 9,1% contra o oracle; diferença estatisticamente significativa, teste de McNemar exato p<0,000002). O que se sustentou, e ficou mais forte, foi H2: o custo de decisão do A* real cresce de 9,3 ms (`sparse`) a 32,7 ms (`very_dense`), contra ~0,055 ms constantes do BC — uma razão de custo de ~600×, medida no mesmo ambiente e nos mesmos trials da taxa de sucesso, mais robusta que a estimativa de ~10× obtida de benchmarks não pareados. A tese sustentada por dados reais, portanto, é que **o ρ-criterion mantém taxa de sucesso próxima da do melhor planejador fixo pagando uma fração pequena do seu custo computacional em alta densidade** — uma versão mais modesta que a formulação original de H1, porém integralmente real, não mais dependente de mock. Esse mesmo padrão (planejador clássico real vencendo em sucesso bruto, com a política aprendida justificada por custo/escalabilidade, não por acerto) é consistente com a evidência multi-agente real da Seção 3.4 (CBS real, 93% RRT*/6% PPO em espaço esparso — o clássico domina onde é barato).
...
- **A motivação original de H1 (taxa de sucesso) não se sustenta sob planejadores reais — reformulada como argumento de custo.** Para reduzir a dependência de mock sem o robô físico, revalidamos H1 com A* real e BC real no gêmeo 2D (Seção 3.3, 1.500 trials, mesma escala do Monte Carlo original): o A* real supera o ρ-criterion e o BC em taxa de sucesso pareada em praticamente todo o espectro de densidade testado (regret real de 9,1% contra o oracle, ante 2,9% do mock; diferença estatisticamente significativa, teste de McNemar exato p<0,000002). A motivação que se sustentou — e ficou mais robusta, com custo medido no mesmo ambiente e trials da taxa de sucesso — é a de H2: o A* real custa até ~600× mais que o BC em alta densidade. Isso é declarado explicitamente como limitação de H1, não escondido: **o resultado quantitativo mais citável deste trabalho (85,3%, H1 na formulação original) permanece calibrado sobre planejadores mock**; a versão sustentada por dados reais é mais modesta (sucesso comparável ao melhor fixo, a uma fração pequena do seu custo), não a formulação original de "supera qualquer método fixo em acerto".
...
- A comutação usa um limiar rígido por restrição de tempo real (Seção 2.2); uma variante de fusão suave/probabilística (mistura contínua ponderada por ρ próximo ao limiar) é uma extensão natural para cenários onde a restrição de tempo real é menos severa.
...
- **Aprendizado por Reforço Multi-Agente (MARL) — próximo passo prioritário, com evidência preliminar real já obtida.** O trabalho estabelece a fundação: formalização do problema como Dec-POMDP com observação local ρᵢ por agente; evidência empírica de que o ρ-criterion produz custo menor que qualquer política uniforme; decisão O(1) por agente que escala para N agentes onde o CBS centralizado não escala. Um primeiro experimento de treino conjunto com recompensa compartilhada (Seção 3.4) — uma política centralizada simplificada, não a arquitetura descentralizada completa — já mostra o padrão esperado: colisão inter-robô cai a 0% (contra 60% do RL independente), com taxa de goal subindo de 25% para 50% conforme o treino avança. O elemento ainda ausente é a arquitetura MARL descentralizada propriamente dita (CTDE); a arquitetura SAC/SB3 já em uso é compatível com essas extensões (QMIX, MADDPG, MAPPO via RLlib).

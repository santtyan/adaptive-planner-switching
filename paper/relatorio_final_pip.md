...
A literatura trata, em sua maioria, a seleção de planejador como decisão fixa de projeto: a linha APPL/APPLR (Xu et al., 2020) aprende por reforço os *parâmetros* de um planejador clássico, não qual planejador usar; o método LiCS (Damanik et al., 2024), vencedor do BARN Challenge 2024, treina uma única política de imitação robusta, sem comutação nenhuma; o HMP-DRL de Kolomeytsev & Golembiovsky (2025) combina um planejador global em grafo com uma política DRL local, mas o global permanece sempre ativo (a política local navega por *checkpoints* que o grafo define), sem um critério explícito de *quando* delegar a decisão a um ou outro. O trabalho mais próximo é o de Sharma et al. (2024), que também comuta entre um planejador clássico (DWA) e uma política de RL por meio de *meta-reasoning*; a diferença está no gatilho, pois a comutação é *reativa* (dispara quando o obstáculo já foi detectado no caminho), enquanto o critério proposto aqui é preditivo. Nossa proposta comuta entre os dois planejadores como alternativas mutuamente exclusivas, governadas por um limiar de densidade explícito. Nenhum desses trabalhos trata a seleção de planejador como variável de otimização contextual *preditiva* — a densidade local ρ, que antecipa a necessidade de comutação **antes** do bloqueio do planejador clássico, não depois de detectá-lo.

Dois pontos da literatura acima merecem nota, por se conectarem diretamente aos nossos próprios resultados. Primeiro, Xu et al. (2020, APPLR) reportam que a vantagem do ajuste adaptativo sobre o planejador fixo **diminui com a dificuldade do ambiente** (57% de ambientes onde APPLR vence em cenários fáceis, contra 28% em cenários difíceis; Tabela III do artigo), atribuindo isso a um teto de desempenho do planejador de base que nenhum ajuste compensa em regime difícil. É o mesmo padrão que observamos ao revalidar a comparação A*/BC no ambiente denso (Seção 3.4): o A* real vence em todos os regimes de densidade testados, mas por margem decrescente conforme a densidade cresce — a literatura já havia documentado essa relação para ajuste de parâmetros; nossos dados mostram que ela também vale para seleção de planejador. Segundo, Sharma et al. (2024) reportam, num sistema estruturalmente próximo ao nosso (DWA clássico + SACPlanner por RL, comutados por heurística reativa), um ganho de 26% no tempo de navegação e 18% de caminho mais curto sobre o método aprendido isolado, com 0% de colisão contra 50–100% do DWA sozinho nos cenários com obstáculo. Diferente do nosso critério, o sistema deles roda os dois planejadores em paralelo o tempo todo e só decide qual saída publicar — o que significa que o ganho de custo computacional que reivindicamos aqui (Seção 3.4, ~656× mais barato o BC em alta densidade) não existe no desenho deles, já que ambos os planejadores são sempre computados. A convergência entre os dois trabalhos (o nosso e o de Sharma et al.) em "comutação supera os métodos isolados" é evidência independente a favor da tese central, mesmo com gatilhos diferentes (densidade preditiva vs. obstáculo reativo).

**Nota de escopo:** a comparação acima não constitui revisão sistemática da literatura (sem protocolo de busca documentado nem critérios de inclusão formais) e não cobre a linha de *algorithm selection*/*hyper-heuristics* (Rice, 1976, e desenvolvimentos posteriores), que trata problema estruturalmente análogo — escolher qual algoritmo executar com base em características da instância; essa lacuna bibliográfica é reconhecida e fica como pendência para a versão submetida ao LAFusion.
...
Para o benchmark dos clássicos, cada algoritmo e tamanho de grid (10×10 a 50×50, densidade 25%, 30 trials/configuração) teve três grandezas medidas: tempo de execução (`timeit.perf_counter()`), memória de pico (`tracemalloc.get_traced_memory()`) e alcançabilidade (fração de trials com origem-destino conectados). Pares de origem/destino são sorteados idênticos entre os quatro métodos em cada grid, o que elimina variância de instância na comparação (script `validation_abstract/benchmark_classical.py`, executado com `--trials 30`; o valor padrão do script é 5, mas a execução registrada em `results_abstract/classical_benchmark.csv` usa 30).
...
A escolha do BC como segundo paradigma, e não uma repetição do SAC com mais tempo de treino, segue de uma diferença estrutural entre os dois: o SAC precisa de milhares de episódios de tentativa e erro para convergir, e o resultado varia de semente para semente (Tabela 4, seção acima). O BC, por sua vez, aprende por imitação direta das ações de um especialista já conhecido (o controlador de campo potencial), o que dispensa a fase de exploração: o treino é curto, o resultado é estável entre execuções, e o custo de decisão em produção é constante (um forward pass), o que o torna um segundo ponto de comparação útil para a Seção 3.4, tanto em taxa de sucesso quanto em custo computacional.
...
Como ilustração qualitativa do mesmo padrão em nível de trajetória individual (`fig_2d_compare_dense.png`, Seção 3.3), um subconjunto de 6 episódios isolados no mundo dense (seeds 0-5, ρ_local médio ≈0,65, script `plot_compare_dense_real.py`) mostra sucesso de 67% para o A*, 83% para o BC e 83% para o adaptativo. A amostra é pequena demais para qualquer teste estatístico próprio e não substitui o resultado principal desta seção (n=1.500); serve apenas para tornar visível, episódio a episódio, o mesmo comportamento medido em agregado acima.
...
*Figura: Episódio real do ρ-criterion (política BC, ρ₀ acima do limiar) navegando o cruzamento urbano com 3 obstáculos dinâmicos simultâneos. Cada obstáculo aparece na posição inicial (contorno tracejado) e final (círculo sólido), ligados por uma seta que indica o sentido do movimento. Entre os episódios bem-sucedidos candidatos, o escolhido para a figura é o de maior mudança de rumo do robô perto de um obstáculo dinâmico (`_deviation_score` em `visualize_urban_dynamic_gif.py`), não o de menor distância: um robô que passa perto sem reagir não ilustra o mecanismo de desvio, mesmo estando fisicamente próximo. Versão animada em `paper/figs/2d/fig_2d_urban_dynamic_multi_episode.gif`.*
